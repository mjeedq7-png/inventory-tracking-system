...
import { getWaste, createWaste, getProducts } from '../../lib/api';
import type { Waste, Product } from '../../types';

export default function OutletWaste() {
  ...
  async function fetchData() {
    try {
      const [wasteResponse, productsResponse] = await Promise.all([
        getWaste(),
        getProducts(),
      ]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.error || 'Failed to record waste');
      }
    } catch {
      ...
      <div className="px-4 py-6 sm:px-0">
        ...
        {showForm && (
          <div className="bg-white shadow rounded-lg p-6 mb-6">
            <h2 className="text-lg font-medium mb-4">Record New Waste</h2>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Reason</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {waste.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-4 text-center text-gray-500">No waste records</td>
                  </tr>
                ) : (
                  waste.map((item) => (
                    <tr key={item.id}>
